...
"...어라?",
"택배 상자 안에 무언가가 있습니다.<br>꺼내보니 종이군요.<br><br><span id='textblue' style='font-size:1.1em;font-weight:bold; color:skyblue'>지금껏 경험하지 못했던 뭐시기 어쩌구 저쩌구를 체험하라!</span><br><br>VR게임 광고지였나봐요. <span style='font-size:0.5em; color:darkgray;'>그것도 진부한...</span><br>마침 할만한 것도 없고, 더 읽어볼까요?",
...
let imgnum = 0;
let textnum = 0;
let imghtml=`<img src='./image/${imgs[imgnum]}' alt=''>`;
    let texthtml=`<div class='textbox'>
                    <img src='textboxtop.png' alt=''>
                    <div class='text'>${text[textnum]}</div>
                    <img src='textboxbottom.png' alt=''>
                </div>`;
...
for(let i=0; textnum<text.length; i++){
    ...
    box.insertAdjacentHTML(
            "beforeend",
            `<img src='./image/${imgs[imgnum]}' alt=''>
            <div class='textbox'>
                <img src='./image/textboxtop.png' alt=''>
                <div class='text'>${text[textnum]}</div>
                <img src='./image/textboxbottom.png' alt=''>
            </div>`
        );
    if(i==1){
        textnum++;
...
document.addEventListener("DOMContentLoaded", function(){
    //main_container.scrollIntoView({behavior: "instant", block: "center"});
    //로딩 영상 포함시킬 것
});
